...
def plot():
    ...
    for game, solver, iterations, label in zip(games, solvers, iterations, labels):
        ...
        for N in [5, 10, 20]:
            ...
            subplot.plot(time_steps[:max_len-1:skip_n], mean_returns[:max_len-1:skip_n], linestyle, color=color, label=f"N={N}", alpha=0.85)
        plt.grid('on')
    ...
    plt.savefig(f'M3FC_training_curves_on_N{dt}.pdf', bbox_inches='tight', transparent=True, pad_inches=0)
