...
def plot():
    ...
    for game, solver, iterations, label, iteration_CTDE, trial_CTDE, solver_CTDE in zip(games, marl_solvers, iterations, labels, iterations_CTDE, trials_CTDE, solvers_CTDE):
        ...
        for N in [5, 10, 20]:
            ...
            for id in range(3):
                ...
                with open(max(files, key=os.path.getctime), 'r') as read_obj:
                    ...
                    for row in csv_reader:
                        ...
                        if len(time_steps) > 500 or len(time_steps) > 100 and i==2:
                            break
            ...
            subplot.plot(time_steps[::skip_n], mean_returns[::skip_n] - std_returns[::skip_n], linestyle, color=color,
                         label='_nolabel_', alpha=0.5)
            subplot.plot(time_steps[::skip_n], mean_returns[::skip_n], linestyle, color=color, label=f"MAPPO N={N}", alpha=0.85)
        ...
        print(game, mean_returns, std_returns)
    ...
    dt = time.time_ns()
    """ Finalize plot """
    plt.gcf().set_size_inches(26, 4)
    plt.tight_layout(w_pad=-0.0, h_pad=0.0)
    lgd = plt.legend(loc="lower right", bbox_to_anchor=(1.9, -0.1))
    plt.savefig(f'mappoMM_MARL_training_curves_new{dt}.pdf', bbox_extra_artists=(lgd,), bbox_inches='tight', transparent=True, pad_inches=0)
