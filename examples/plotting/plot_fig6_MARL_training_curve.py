...
def evaluate_objective_N_MFC(game, iterations, solver, num_return_trials, Ns, trial, exp):
    if exp == 'exp5':
        map_name = 'exp0'
    elif exp == 'exp6':
        map_name = 'exp1'
...
def plot():
    ...
    iterations = [500, 500, 500, 300, 500]
    ...
    for game, solver, iterations, label, iteration_CTDE, trial_CTDE, solver_CTDE in zip(games, marl_solvers, iterations,
                                                                                        labels, iterations_CTDE,
                                                                                        trials_CTDE, solvers_CTDE):
        # if game == 'major-minor-foraging':
        if game == 'comms-two':
            exp = 'exp5'
        ...
        for N in [20]:
            ...
            subplot.plot(time_steps[::skip_n], mean_returns[::skip_n] - std_returns[::skip_n], linestyle, color=color,
                         label='_nolabel_', alpha=0.5)
            subplot.plot(time_steps[::skip_n], mean_returns[::skip_n], linestyle, color=color, label="IPPO", alpha=0.85)
        ...
        all_mf_returns = []
        for id in range(3):
            file_path = mappo_marllib_path + 'mappo_mlp_{}_{}_{}_mappo'.format(exp, N, id)
            config = generate_config_from_kw(**{
                'game': game,
                'solver': solver,
                'cores': 1,
        ...
        subplot.plot(time_steps[::skip_n], mean_returns[::skip_n] - std_returns[::skip_n], linestyle, color=color,
                     label='_nolabel_', alpha=0.5)
        subplot.plot(time_steps[::skip_n], mean_returns[::skip_n], linestyle, color=color, label="MAPPO",
                     alpha=0.85)
        subplot.fill_between(time_steps[::skip_n], mean_returns[::skip_n] - std_returns[::skip_n],
                             mean_returns[::skip_n] + std_returns[::skip_n], color=color, alpha=0.15)
        ...
        J_Ns_MFC = evaluate_objective_N_MFC(game, iteration_CTDE, solver_CTDE, num_return_trials, Ns, trial_CTDE, exp)
        ...
        if len(mean_returns) == 1:
            subplot.plot([0, time_steps[-1]], [mean_returns[0]] * 2, linestyle, color=color, label="MF",
                         alpha=0.85, linewidth=5.0)
        else:
            subplot.plot([0, time_steps[-1]], [mean_returns[5]] * 2, linestyle, color=color, label="MF", alpha=0.85,
                         linewidth=5.0)
    ...
    plt.savefig(f'MM_MARL_training_curves_new{dt}.pdf',
                bbox_extra_artists=(lgd,), bbox_inches='tight', transparent=True, pad_inches=0)
