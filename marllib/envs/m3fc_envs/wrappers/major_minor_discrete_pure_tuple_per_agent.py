...
class PureDiscreteTupleMajorMinorMFCEnvPerAgent(MajorMinorMFCEnv):
    ...
    def sample_actions(self, action):
        ...
        for i in range(self.env_MARL.num_agents):
            ...
            cum_ps = np.concatenate(
                [np.zeros((self.env_MARL.num_agents, 1)), cum_h[tuple(self.env_MARL.xs.transpose())]], axis=-1)
            ...
            actions_minor[i] = self.u_bin_centers[actions_idxs][i]
