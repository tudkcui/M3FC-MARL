...
class MARLObsWrapperComms(MultiAgentEnv):
    """
    Wraps a mean-field MARL problem in discrete time with extra observations.
    """
    def __init__(self, env_MARL: MeanFieldMARLEnv, x_bins_per_dim: int = 5, unbounded_actions=False,
                 obs_time=0, periodic_time=False, **kwargs, ):
        super().__init__()
        self.env_MARL = env_MARL
        self.x_dims = self.env_MARL.observation_space.shape[0]
        self.u_dims = self.env_MARL.action_space.shape[0]
        self.x_low = self.env_MARL.observation_space.low
        self.x_high = self.env_MARL.observation_space.high
        self.u_low = self.env_MARL.action_space.low
        self.u_high = self.env_MARL.action_space.high
        ...
        self._agent_ids = list(range(self.env_MARL.num_agents))

        self.x_bins_per_dim = x_bins_per_dim
        self.num_obs_layers = 1 + self.x_dims + self.num_dims_time + 1
        ...
        # self.observation_space = Box(0, 1, shape=(np.prod((x_bins_per_dim,) * self.x_dims
        #                                                             + (self.num_obs_layers,)).item(),), dtype=np.float64)
        self.observation_space = MultiAgentObservationSpace([Box(0, 1, shape=(np.prod((x_bins_per_dim,) * self.x_dims + (self.num_obs_layers,)).item(),),
                                    dtype=np.float32) for _ in range(self.env_MARL.num_agents)])
        # self.action_space = MultiAgentActionSpace([self.env_MARL.action_space for _ in range(self.env_MARL.num_agents)])
        self.action_space = MultiAgentActionSpace([Box(low=self.u_low, high=self.u_high, shape=(self.u_dims,), dtype=np.float32)
                                              for _ in range(self.env_MARL.num_agents)])
        self.num_agents = self.env_MARL.num_agents
    ...
    def get_observation(self):
        normalized_xs = (self.env_MARL.xs - self.x_low) / (self.x_high - self.x_low)
        ...
        if self.obs_time:
            ...
            return {i: np.concatenate([np.tile(normalized_xs[i], (self.x_bins_per_dim,) * self.x_dims + (1,)),
                                       obs_agents, obs_users, obs_time], axis=-1).flatten().astype(np.dtype("float32")) for i in range(self.env_MARL.num_agents)}
        else:
            return {i: np.concatenate([np.tile(normalized_xs[i], (self.x_bins_per_dim,) * self.x_dims + (1,)),
                                       obs_agents, obs_users], axis=-1).flatten().astype(np.dtype("float32")) for i in range(self.env_MARL.num_agents)}
    ...
    def step(self, actions):
        if self.unbounded_actions:
            actions.update((k, (np.tanh(actions[k]) + 1) / 2 * (self.u_high - self.u_low) + self.u_low)
                           for k in actions.keys())
        xs, rewards, dones, _ = self.env_MARL.step(actions)
    ...
    def action_space_sample(self, agent_ids: list = None):
        return {i: self.action_space.sample() for i in range(self.env_MARL.num_agents)}
