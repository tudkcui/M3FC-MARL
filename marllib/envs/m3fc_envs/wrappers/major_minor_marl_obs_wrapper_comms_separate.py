...
class MajorMinorMARLObsWrapperCommsSeparate(MultiAgentEnv):
    """
    ...
    def __init__(self, env_MARL: MajorMinorMARLEnv, x_bins_per_dim: int = 5, unbounded_actions=False,
        ...
        # self.observation_space = MultiAgentObservationSpace([Tuple([
        #     self.env_MARL.minor_observation_space,
        #     self.env_MARL.major_observation_space,
        #     Box(0, 1, shape=(np.prod((x_bins_per_dim,) * self.x_dims + (self.num_obs_layers,)).item(),), dtype=np.float32),
        # ]) for _ in range(self.env_MARL.num_agents)])
        # self.action_space = MultiAgentActionSpace([Box(low=self.u_low, high=self.u_high, shape=(self.u_dims,), dtype=np.float64)
        #                                       for _ in range(self.env_MARL.num_agents)])
        self.num_agents = len(self._agent_ids)
        self.observation_space = Tuple([
            self.env_MARL.minor_observation_space,
            self.env_MARL.major_observation_space,
            Box(0, 1, shape=(np.prod((x_bins_per_dim,) * self.x_dims + (self.num_obs_layers,)).item(),), dtype=np.float64),
        ])
    ...
    def step(self, actions):
        ...
        act = {i: actions[i] for i in actions if i!=-1}, actions[-1]
